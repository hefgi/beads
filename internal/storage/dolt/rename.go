...
// Handles both regular issues (issues table) and wisps (wisps table).
...
func (s *DoltStore) UpdateIssueID(ctx context.Context, oldID, newID string, issue *types.Issue, actor string) error {
	...
	// Try updating in the issues table first
	now := time.Now().UTC()
	...
	isWisp := rows == 0
	if isWisp {
		// Issue not in issues table — try the wisps table
		result, err = tx.ExecContext(ctx, `
			UPDATE wisps
			SET id = ?, title = ?, description = ?, design = ?, acceptance_criteria = ?, notes = ?, updated_at = ?
			WHERE id = ?
	...
	// Update references in the appropriate tables based on where the issue lives.
	if isWisp {
		// Update wisp-specific tables
